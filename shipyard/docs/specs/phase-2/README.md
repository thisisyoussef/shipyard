...
4. Keep later tool contracts and outputs stream-friendly for the browser UI that gets introduced immediately after `P2-S02`.

## Shared Constraints

- Product code and product docs stay under `shipyard/`; `.ai/` remains helper-only.
- `P2-S01` and `P2-S02` land before the UI pack. From `P2-S03` onward, tool outputs should stay easy to summarize into activity rows, diff panels, and status updates rather than hiding useful detail in opaque objects.
...
|---|---|---|---|
| P2-S01 | Registry and Anthropic Tool Export | Define the shared tool contract, self-registration flow, and model-facing schema projection. | Phase 1 implementation |
| P2-S02 | Safe Relative File IO | Implement `read_file` and `write_file` around shared path and hash tracking primitives. | P2-S01 |
| P2-S03 | Surgical `edit_block` Guardrails | Enforce stale-read detection, anchor disambiguation, and diff-size guardrails. | P2-S02, Phase Pre-2 implementation |
| P2-S04 | Discovery, Execution, and Smoke Coverage | Finish `list_files`, `search_files`, `run_command`, `git_diff`, and the direct tool smoke script. | P2-S01, Phase Pre-2 implementation |
...
- Phase Pre-2 lands immediately after `P2-S02`, so the remaining tool work can assume a browser surface exists and can shape outputs for both model use and live UI streaming.
...
- Tool outputs are concise and structured enough to appear naturally in the browser UI's activity and diff surfaces.
