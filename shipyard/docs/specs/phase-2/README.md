...
4. Keep tool contracts and outputs stream-friendly for the browser UI introduced in Phase Pre-2.

## Shared Constraints

- Product code and product docs stay under `shipyard/`; `.ai/` remains helper-only.
- Phase Pre-2's browser UI is assumed to exist first, so tool outputs should stay easy to summarize into activity rows, diff panels, and status updates rather than hiding useful detail in opaque objects.
...
|---|---|---|---|
| P2-S01 | Registry and Anthropic Tool Export | Define the shared tool contract, self-registration flow, and model-facing schema projection. | Phase Pre-2 implementation |
...
- Phase Pre-2 lands before this pack so tool outputs are shaped for both model use and browser streaming from the beginning.
