...
4. Feed raw-loop progress into the browser UI so developers can watch Claude think and act naturally.
...
|---|---|---|---|
| P3-S01 | Anthropic Client and Tool-Use Contract | Add the minimal Claude Messages API client, model/config contract, and message-block helpers the raw loop depends on. | Phase 2 implementation, Phase Pre-2 implementation |
...
- The same loop activity can stream into the browser UI without inventing a second observability path.
