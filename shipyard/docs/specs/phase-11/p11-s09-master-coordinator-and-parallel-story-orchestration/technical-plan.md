...
- Public interfaces/contracts:
  ...
  - `CoordinatorDecision`
  - `MergeConflictResolutionState`
  - `HostedWorkerTarget`
  - `CoordinatorStatusProjection`
  - `HumanIntervention`
- Data flow summary: the master coordinator watches the task graph, chooses
  ready tasks, assigns them to role-aware workers, waits on approvals, lease
  conflicts, source-control blockers, or hosted-capacity blockers when
  necessary, accepts human interventions, routes GitHub merge work to the
  dedicated PR-ops role, and projects current runtime status for later UI
  consumption.
...
- Story ordering rationale: this story sits last because it depends on the full
  artifact, approval, profile, TDD, source-control, hosted-runtime, and
  task-graph stack.
...
- Decision: adopt a first-merge-wins policy and make later conflicting work a
  first-class coordinator recovery path.
- Alternatives considered:
  - serialize all merges so only one story can complete at a time
  - let parallel branches merge optimistically and rely on manual cleanup later
- Rationale: strict serialization wastes too much parallelism, while optimistic
  merge-without-recovery is how hidden divergence turns into operator pain.
...
- Request shape:
  ...
  - apply human interrupts such as stop, reprioritize, inject feedback, approve,
    or reroute
  - request conflict recovery or rebind from degraded-source mode
- Response shape:
  - active worker assignments
  - blocked or waiting reasons
  - source-control or hosted-runtime blockers
  - conflict-resolution queue entries
...
- Storage/index changes:
  ...
  - conflict recovery records linked to source-control refs and story nodes

## Dependency Plan
- Existing dependencies used: artifact registry, pipeline runner, skill and
  profile registry, PM backlog, TDD lane, GitHub source-control state, hosted
  Railway runtime state, task graph, current `ultimate mode` loop and session
  persistence.
...
- Risk and mitigation:
  ...
  - Risk: stale branches keep running after another branch merged first.
  - Mitigation: scheduler revalidates default-branch freshness before merge or
    apply steps and opens explicit recovery tasks when staleness is detected.

## Test Strategy
- Unit tests:
  - scheduler readiness and dependency checks
  - worker assignment selection
  - source-control blocker handling
  - hosted capacity handling
  - human intervention handling
- Integration tests:
  - schedule two ready stories with different role needs
  - block work on approval, lease conflict, or hosted-capacity conflict
  - enforce first-merge-wins and route stale branches into recovery
...
- Edge-case coverage mapping:
  ...
  - GitHub unavailable but degraded local mode still active
  - failed worker while others continue
  - human reprioritization mid-run
  - merged-first branch invalidates later worker
...
- Feature flags/toggles: enable multi-story orchestration only after lower-level
  task graph, source-control, and hosted-runtime contracts are proven.
- Observability checks: log scheduling decisions, worker churn, blocked reasons,
  intervention events, merge staleness transitions, hosted-capacity state, and
  orchestration resume points.
