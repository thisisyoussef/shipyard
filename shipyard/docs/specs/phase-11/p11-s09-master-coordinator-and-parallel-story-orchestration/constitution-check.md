...
- [x] New modules respect SRP and dependency direction. The master coordinator,
  worker scheduling, dependency resolution, source-control recovery, hosted
  capacity handling, and task apply policy remain distinct layers.
...
- [x] Testing strategy covers the change. Scheduling, dependency blocking,
  lease-aware assignment, human interrupts, first-merge-wins recovery, and
  resume behavior all need dedicated coverage.
- [x] New dependency risk is bounded. Initial orchestration should reuse
  ...
  introducing a heavyweight external scheduler or cloud-specific orchestration
  plane.
