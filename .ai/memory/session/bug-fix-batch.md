...
| BUG-005 | Railway production still runs Anthropic even when operators expect the hosted service to be pinned to OpenAI. | The Railway deploy workflow should sync `OPENAI_API_KEY` plus an OpenAI provider/model pin into the hosted production service. | Operator production error report showing Anthropic overloads; deploy contract in `.github/workflows/railway-main-deploy.yml` and `shipyard/docs/architecture/hosted-railway.md` | `shipyard/tests/railway-config.test.ts` plus hosted-doc contract review | `.github/workflows/railway-main-deploy.yml`, `shipyard/tests/railway-config.test.ts`, `shipyard/docs/architecture/hosted-railway.md`, `shipyard/docs/architecture/README.md`, `shipyard/README.md`, `shipyard/docs/specs/phase-9/README.md`, `shipyard/docs/specs/phase-provider-agnostic-runtime/README.md`, `.ai/docs/SINGLE_SOURCE_OF_TRUTH.md`, `.ai/memory/project/patterns.md`, `.ai/memory/project/architecture.md`, `.ai/memory/codex/README.md` | Fixed |
| BUG-006 | The GitHub Actions Railway deploy chooses `RAILWAY_API_TOKEN` ahead of `RAILWAY_TOKEN`, so deploys fail at `railway project link` when the API token lacks access but the project token is valid. | The workflow should prefer `RAILWAY_TOKEN` for project-scoped deploy control and only fall back to `RAILWAY_API_TOKEN` when no project token is configured. | Failed GitHub Actions run `23719471449` on PR #179: `Unauthorized` at `Link Railway production project`, plus the hosted contract in `shipyard/docs/architecture/hosted-railway.md` already saying project token first | `shipyard/tests/railway-config.test.ts` token-selection regression plus rerun of the workflow after merge | `.github/workflows/railway-main-deploy.yml`, `shipyard/tests/railway-config.test.ts`, `shipyard/docs/specs/phase-9/README.md`, `.ai/memory/session/decisions-today.md` | Fixed |
...
- Focused Railway provider regression: `pnpm --dir shipyard exec vitest run tests/railway-config.test.ts`
- Full validation: `pnpm --dir shipyard test && pnpm --dir shipyard typecheck && pnpm --dir shipyard build && git diff --check`
- LangSmith finish check: blocked in this worktree because `OPENAI_API_KEY`
  and a LangSmith project name are not configured locally, so no live OpenAI
  verification trace could be produced.
